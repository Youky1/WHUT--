...
Page({
    ...
    inputPhoto(){
        ...
        let count = 3 - newArray.length//还可以添加count张图片
        var self = this;
        wx.chooseImage({
            'count': count,
            success: (res) => {
    ...
    release(e) {
        ...
        if(!form.name){
            ...
            let self = this;
            let app = getApp();
            let sendP = app.sendAllArray(this.data.inputImageArray,0,'');
            sendP.then(str => {
                console.log('最终上传的url为： ',str);
                form.picUrls = str;
                ...
                wx.request({
                    ...
                    url:'http://123.57.249.95:8090/secondhand/goods/publish',
                    data: form,
                    success: (res)=> {
                        //发布成功
